...
using PinkDao;
using PinkoServices.BaseMessageHandlers;
        ...
        public void Listen()
        {
            ...
            while (_isRunning && !PinkoApplication.ApplicationRunningEvent.WaitOne(0))
            {
                Exception ex = TryCatch.RunInTry(() =>  
                {
                    ...
                    {
                        // Process the message
                        Trace.WriteLine("Recevied: ", receivedMessage.Verbose());

                        // Send to listeners
                        _messageHandlerManager.Publish(new AzureBrokeredMessageEnvelopInbound(receivedMessage)
                                                                   {
                                                                       QueueName = QueueName
                                                                   });
